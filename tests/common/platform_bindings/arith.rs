use system_r::{terms::{Term, Literal, Kind}, platform_bindings::WrappedContent, diagnostics::Diagnostic, types::{Context, Type}};
...
pub fn pull_u32_from(args: &Vec<Term>, idx: usize, span: &Span) -> Result<u32, Diagnostic> {
    let arg_t_raw = match args.get(idx) {
        Some(t) => t,
        None => return Err(Diagnostic::error(*span, format!("pull_u32_from: nothing in arg slot for idx {}", idx)))
    };
    let arg_actual = match arg_t_raw.kind.clone() {
        Kind::Lit(Literal::Nat(n)) => n,
        k => return Err(Diagnostic::error(*span, format!("pull_u32_from: expected a Nat lit kind, got {:?}", k)))
    };
    Ok(arg_actual)
}
...
fn add(arg: Term, span: &Span) -> Result<Term, Diagnostic> {
    match arg.kind {
        ...
        _ => return Err(Diagnostic::error(arg.span(), format!("nat::arith::add: Expected a product argument")))
    };
}