...
use crate::syntax::parser::{ExtParser, ParserExtension};
/*
...
pub fn type_check_term(ctx: &mut types::ExtContext<BottomPattern, BottomKind, BottomExtension>, term: &mut ExtTerm<BottomPattern, BottomKind>) -> Result<Type, Diagnostic> {
    // Step 1
    let ty = ctx.type_check(&term)?;
    Ok(ty)
}

pub fn dealias_and_type_check_term(ctx: &mut types::ExtContext<BottomPattern, BottomKind, BottomExtension>, term: &mut ExtTerm<BottomPattern, BottomKind>) -> Result<Type, Diagnostic> {
    // Step 0
    ctx.de_alias(term);
...
pub fn operate_parser_for<'s, TExtTokenKind: Clone + fmt::Debug + Default + PartialEq + PartialOrd,
        ...
        TPE: Clone + ParserExtension<TExtTokenKind, TExtKind>>(mut parser: ExtParser<'s, TExtTokenKind, TExtKind, TEXtPat, TLE, TPE>, input: &str) -> Result<ExtTerm<TEXtPat, TExtKind>, Diagnostic> {
...
pub fn type_check_and_eval_single_block(ctx: &mut types::ExtContext<BottomPattern, BottomKind, BottomExtension>,
