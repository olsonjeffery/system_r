...
use crate::{types::Type, terms::ExtTerm, patterns::ExtPattern};
...
pub trait SystemRExtension<TExtTokenKind: fmt::Debug + PartialEq + PartialOrd + Default + Clone,
    ...
    fn lexer_lex_ext_keyword(&mut self, data: &str) -> TExtTokenKind;
    fn pat_ext_pattern_type_eq(&self, pat: &TExtPat, ty: &Type) -> bool;
    fn pat_add_ext_pattern<'a>(&'a self, parent: &crate::types::patterns::Matrix<'a, TExtPat, TExtKind>, ext_pattern: &ExtPattern<TExtPat, TExtKind>) -> bool;
