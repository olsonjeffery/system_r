//! Convert a Term<TExtDialect> to system_r plaintext (srpt) format; needs to be implemented for each TExtDialect
use anyhow::Result;
...
pub trait Plaintext<TExtDialect: SystemRDialect> {
    ...
    fn handle_ext(&self) -> Result<String> {
        Err(anyhow!("handle_ext should be overriden in implementors (except BottomDialect)"))
    }
}