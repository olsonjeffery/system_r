...
fn sub(arg: Term<BottomDialect>, span: &Span) -> Result<Term<BottomDialect>> {
    match arg.kind {
        Kind::Product(args) => {
            if args.len() != 2 {
                return Err(
                    TypeCheckerDiagnosticInfo::error(*span, "nat::arith::sub: expected product of len 2").into(),
                );
            }
        ...
        _ => {
            return Err(TypeCheckerDiagnosticInfo::error(
                arg.span(),
                format!("nat::arith::sub: Expected a product argument"),
            )
            .into())
        }
    };
...
fn add(arg: Term<BottomDialect>, span: &Span) -> Result<Term<BottomDialect>> {
    match arg.kind {
        Kind::Product(args) => {
            if args.len() != 2 {
                return Err(
                    TypeCheckerDiagnosticInfo::error(*span, "nat::arith::add: expected product of len 2").into(),
                );
            }
            let arg0_actual = pull_u64_from(&args, 0, span)?;
            let arg1_actual = pull_u64_from(&args, 1, span)?;
